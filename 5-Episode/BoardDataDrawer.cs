...
public class BoardDataDrawer : Editor
{
   ...
   private void InitializeReordableList(ref ReoderableList list, string propertyName, string listLabel)
   {
      list = new ReoderableList(serializedObject, serializedObject.FindProperty(propertyName), 
         true, true, true, true);
      ...
      if(GUILayout.Button("Fill Up With Random"))
      {
         for (int i = 0; i < GameDataInstance.Columns; i++)
         {
            for (int j = 0; j < GameDataInstance.Rows; j++)
            {
               ...
               string letters = "ABCÇDEFGHIJKLMNOPQRSTUVWXYZ";
